...
<!-- keel:project:start -->
<!-- auto-generated by keel · 2026-04-04 04:08 UTC · do not edit this block -->
## Project Decisions (fraud-dev-v4)
**GPU inference for GNN+XGBoost**: Combines both models into a single Triton Python backend with dynamic CUDA detection, moving tensors to GPU and setting XGBoost device=cuda:0 to maximize performance while ensuring CPU fallback.

**Dynamic UI configuration**: Uses Kubernetes ConfigMap-served JSON for runtime-configurable titles, logos, and customer-facing copy, enabling instant updates without image rebuilds.

**Fraud metrics refresh**: Moves fraud metrics into `collect_fast()` (200ms cycle, 5s cache TTL) to decouple from slow `collect()` and ensure 5-7s updates regardless of load.

**Graph data display**: Applies windowing (max aggregation) only to dashboard cards while showing real-time values in infrastructure graphs for stable summaries and dynamic monitoring.

**Feature alignment**: Ensures model-build includes all 35 scorer features (including 14 derived metrics) to maintain compatibility with Triton and avoid runtime errors.

## Cross-Project Principles
**GPU efficiency**: Prioritizes GPU acceleration for compute-heavy tasks (e.g., inference) but avoids premature optimization for smaller workloads, balancing performance and complexity.

**Configuration decoupling**: Separates runtime-configurable elements (UI text, logos) from code to reduce deployment friction and enable faster updates.

**Monitoring granularity**: Uses shorter windows (10s) for GPU/utilization metrics to detect transient spikes while maintaining longer windows for stable aggregates.
<!-- keel:project:end -->
