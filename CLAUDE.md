...
```powershell
# Apply demo-config ConfigMap first (required by backend — must exist before pod starts)
...
<!-- keel:project:start -->
<!-- auto-generated by keel · 2026-04-04 14:08 UTC · do not edit this block -->
## Project Decisions (fraud-dev-v4)
**Phase 2 warm-up progress**: Waits for the first scoring batch before dismissing the pod modal to show accurate progress, improving transparency and preventing premature dismissal.
**Dashboard title source**: Uses only `demo-config.yaml` for dashboard titles, aligning with explicit project assumptions and avoiding configuration drift.
**Conditional pipeline checks**: Validates "all green" status only when transaction volume per second > 0, reducing overhead and avoiding false negatives during idle states.
**WebSocket for phase2 completion**: Relies on `total_transactions` via WebSocket instead of score file counts for real-time, accurate phase2 completion signals.
**GPU chart standardization**: Assigns distinct colors to GPU metrics (scoring: purple, Triton: blue, prep: orange, training: green) and matches tile colors for visual consistency.
**Triton hot-reload**: Automatically reloads Triton after each training cycle using the model management API to ensure immediate model updates.
**GPU role mapping**: Uses Kubernetes API to map GPU metrics by pod role, ensuring stable dashboard tiles across restarts and eliminating role metadata inconsistencies.

## Cross-Project Principles
**Deterministic GPU allocation**: Pins GPU devices per pod via `CUDA_VISIBLE_DEVICES` in deployment YAML to prevent index swaps and ensure workload consistency.
**Unified model serving**: Combines GNN and XGBoost models in a single Triton Python backend for simplified deployment and reduced operational overhead.
**Real-time vs. windowed data**: Applies windowing only to card tiles while displaying real-time values in infrastructure graphs, balancing accuracy and performance.
<!-- keel:project:end -->
